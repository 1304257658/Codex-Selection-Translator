...
- 原文和译文各自提供喇叭按钮，使用 Chromium 的系统语音朗读，再次点击可停止播放。
...
本项目通过仅监听本机回环地址 `127.0.0.1` 或 `[::1]` 的 Chrome DevTools Protocol（CDP）向 Codex 渲染页面注入界面。本地翻译直接在 Codex 的 Chromium 渲染器中执行；Google/Bing 请求由本机 Node.js 进程发送。
...
- CDP 可以执行 Codex 渲染器中的 JavaScript 并读取当前页面内容。本项目将调试端口限制在本机回环地址，但本机其他进程仍可能访问它。
