...
Chromium 不提供直接列出全部语言包的接口，因此本工具会保存已确认语言对。该记录为空（例如首次安装、设置文件丢失）时，启动后会通过 `Translator.availability()` 自动检查支持列表中的语言对并重建记录，无需逐个手动验证。如果浏览器更新或清理了模型数据，仍可在设置中点击“重新验证翻译包”。
